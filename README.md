...
```bash
> docker run --rm -p "8005:8005" becorecode/curso-intro-docker-modulo-5:latest
```

Una vez levantado el contenedor, accede con un navegador web a `http://localhost:8005`
...
* Para imprimirlas:
  * Accede a la URL `http://localhost:8005/?print-pdf`
  * Imprime la página 
  * Si deseas añadir las notas del presentador utiliza la URL  `http://localhost:8005/?print-pdf&showNotes=true`
...
Puedes acceder a las diapositivas en `localhost:8005`. Haz los cambios que necesites en las diapositovas situadas en la carpeta
